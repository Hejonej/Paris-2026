const C='afield-1-8-refine-20260919-1458';
...
self.addEventListener('fetch',e=>{
  if(e.request.method==='GET'&&e.request.mode==='navigate'){
    e.respondWith((async()=>{
      ...
      if(!html.includes('__afieldRefineCard')){
        try{
          const refine=await fetch(new URL('./refine.js?v=20260919-1458',self.location.href),{cache:'no-store'}).then(x=>x.text());
          html=html.replace('</body>',`<script>${refine}<\/script></body>`);
        }catch(err){
          html=html.replace('</body>',`<script src="./refine.js?v=20260919-1458"><\/script></body>`);
        }
      }
